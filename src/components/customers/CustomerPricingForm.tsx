...
export const CustomerPricingForm = ({ customerId, onSuccess }: CustomerPricingFormProps) => {
  ...
  const handleSubmit = async () => {
    ...
    const validEntries = entries.filter(entry => 
      entry.product_id && entry.price > 0
    );
    ...
    try {
      ...
      console.error('Failed to create pricing entries:', error);
    }
  };
                  ...
                  <SelectContent>
                    {products.map((product) => (
                      <SelectItem key={product.id} value={product.id}>
                        {product.name} ({product.sku}) - Base: KD {product.base_price.toFixed(2)}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                ...
                <div className="space-y-1">
                  ...
                  {entry.product_id && (
                    <p className="text-xs text-gray-500">
                      Base price: KD {getProductBasePrice(entry.product_id).toFixed(2)}
                    </p>
                  )}
                </div>
              </div>
